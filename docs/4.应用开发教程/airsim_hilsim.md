...
注意：本教程是我们 XDKJY DASA service 服务的组成部分
...
本教程为感兴趣的用户提供了一个起点，用户可以熟悉 Airsim 环境并了解其如何在硬件级别与我们的 XDKJY 自动驾驶仪进行交互。

## 先决条件

下面列出了需要的工具（已进行全面测试）：

- XDKJY autopilot ：固件版本为v1.10.0以上
- 一个 RC 接收器和一个遥控器
- 算力强大的个人电脑
...
XDKJY 自驾仪的HIL模式可以参照 https://docs.px4.io/main/en/simulation/hitl.html 中的说明进行设置。简而言之，启用HIL模式，QGround地面站应该只配置 UDP通信。自动驾驶仪将通过自身USB 端口与 Airsim 平台交互mavlink 消息。
